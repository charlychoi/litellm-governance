...
## Scenario 6 — Claude/Codex 실제 provider 경유 검증 ⚑ 필수

### 목적

CLI에서 mock이 아닌 실제 alias 모델을 호출할 수 있는지 확인한다.
이 시나리오가 Phase 2.3의 핵심 명제: CLI 도구가 실제 외부 LLM 호출 시에도 가상키 정책이 유지되는가.
...
- CLI 응답 성공 (mock 문구 없는 실제 LLM 응답)
- LiteLLM 로그에 `dev-kim-codex`
- 모델 alias 기록
- token/spend 기록
- 비허용 모델로 CLI 호출 시 차단 확인
